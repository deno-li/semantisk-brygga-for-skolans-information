...
interface InterventionEffect {
  immediate: number;      // Effect in first month
  shortTerm: number;      // Effect after 3 months
  longTerm: number;       // Effect after 6+ months
  variability: number;    // How much the effect can vary (0-1)
}
...
const INTERVENTIONS: Intervention[] = [
  ...
  },
  {
    id: 'art-therapy',
    name: 'Bildterapi',
    description: 'Kreativ terapi genom konst och bildskapande',
    category: 'individual',
    emoji: '🎨',
    duration: '3 månader',
    cost: 'medium',
    staffRequired: 1,
    effects: {
      healthy: { immediate: 0.3, shortTerm: 0.5, longTerm: 0.6, variability: 0.25 },
      nurtured: { immediate: 0.2, shortTerm: 0.4, longTerm: 0.5, variability: 0.2 },
      achieving: { immediate: 0.1, shortTerm: 0.3, longTerm: 0.4, variability: 0.3 },
      respected: { immediate: 0.2, shortTerm: 0.3, longTerm: 0.4, variability: 0.2 }
    }
  },
  {
    id: 'parent-training',
  ...
  },
  {
    ...
    name: 'Krisinsats',
    description: 'Snabb och intensiv insats vid akut behov',
    category: 'individual',
    emoji: '🚨',
    duration: '2-4 veckor',
    cost: 'high',
    staffRequired: 2,
    effects: {
      safe: { immediate: 0.8, shortTerm: 0.6, longTerm: 0.4, variability: 0.3 },
      healthy: { immediate: 0.5, shortTerm: 0.4, longTerm: 0.3, variability: 0.25 },
      nurtured: { immediate: 0.4, shortTerm: 0.3, longTerm: 0.2, variability: 0.3 }
    }
  }
];
...
const CATEGORY_CONFIG = {
  individual: { name: 'Individuella', color: 'indigo', icon: Users },
  family: { name: 'Familj', color: 'rose', icon: Heart },
  school: { name: 'Skola', color: 'amber', icon: Target },
  community: { name: 'Samhälle', color: 'emerald', icon: Users }
};

const COST_CONFIG = {
  low: { label: 'Låg', color: 'emerald', icon: '💚' },
...
const runSimulation = (
  ...
  // Calculate confidence based on intervention's effect variability
...
// ============================================
// INTERVENTION SIMULATOR COMPONENT
// ============================================
export const InterventionSimulator: React.FC = () => {
  ...
  const [expandedCategory, setExpandedCategory] = useState<string | null>('individual');
  ...
  const runAllSimulations = useCallback(() => {
    ...
    // Simulate with a slight delay for visual feedback
    setTimeout(() => {
  ...
  // Calculate combined effects if multiple interventions selected
  const combinedProjection = useMemo(() => {
    ...
    simulationResults.forEach(result => {
      SPOKES.forEach(spoke => {
        ...
        combined[spoke.id] = Math.min(5, combined[spoke.id] + improvement * 0.7); // Diminishing returns for multiple
      });
    });
  ...
  const totalCost = useMemo(() => {
    ...
    return selectedInterventions.reduce((sum, id) => {
      const intervention = INTERVENTIONS.find(i => i.id === id);
      return sum + (intervention ? costs[intervention.cost] : 0);
    }, 0);
  ...
  const renderInterventionSelector = () => (
          ...
          <div key={category} className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            <button
              ...
              className={`w-full flex items-center justify-between p-4 transition-colors ${
                isExpanded ? `bg-${config.color}-50` : 'hover:bg-gray-50'
              }`}
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <h3 className="font-semibold text-gray-900">{config.name} insatser</h3>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-2 mt-3">
                          ...
                          <span className={`text-xs px-2 py-1 rounded-full bg-${COST_CONFIG[intervention.cost].color}-100 text-${COST_CONFIG[intervention.cost].color}-700`}>
                            {COST_CONFIG[intervention.cost].icon} {COST_CONFIG[intervention.cost].label} kostnad
                          </span>
  ...
  const renderSimulationResults = () => {
    if (!showResults || simulationResults.length === 0) return null;
      ...
      <div className="mt-6 space-y-6">
                ...
                <div key={result.interventionId} className="bg-white rounded-xl p-4 border border-gray-200">
                  ...
                  <div className="flex items-center gap-1 overflow-x-auto">
                    {result.timeline.map((point, i) => (
                      ...
                      >
                        <div className="text-[10px] text-gray-500">M{point.month}</div>
                        <div className="h-8 bg-gray-100 rounded relative mt-1">
                          <div
                            className="absolute bottom-0 w-full rounded transition-all"
                            style={{
                              height: `${(Object.values(point.scores).reduce((a, b) => a + b, 0) / (SPOKES.length * 5)) * 100}%`,
                              backgroundColor: intervention.emoji === '🚨' ? '#EF4444' : '#10B981'
                            }}
                          />
                        </div>
                      </div>
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-6 border border-blue-200">
          ...
          <div className="space-y-3">
            ...
            {totalCost >= 4 && (
              <div className="flex items-start gap-3 p-3 bg-rose-100 rounded-lg">
                <DollarSign className="w-5 h-5 text-rose-600 flex-shrink-0 mt-0.5" />
                <div>
                  <p className="font-medium text-rose-900">Hög total kostnad</p>
                  <p className="text-sm text-rose-700">
                    Överväg att prioritera insatser eller dela upp över tid.
                  </p>
                </div>
              </div>
            )}
    ...
    <div className="bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 rounded-3xl p-6 shadow-xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Insatssimulator</h2>
            <p className="text-gray-600">Testa olika insatser och se förväntade resultat</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          {selectedInterventions.length > 0 && (
            <div className="flex items-center gap-4">
              ...
              <div className={`text-sm ${totalCost > 4 ? 'text-rose-600' : 'text-gray-600'}`}>
                Total kostnad: {'💰'.repeat(Math.min(totalCost, 5))}
              </div>
            </div>
          )}
        </div>
      ...
      {selectedInterventions.length > 0 && !showResults && (
        <button
          ...
          className="w-full mt-6 flex items-center justify-center gap-3 px-6 py-4 bg-gradient-to-r from-indigo-500 to-purple-600 text-white font-semibold rounded-2xl hover:opacity-90 transition-opacity shadow-lg disabled:opacity-50"
        >
          {isSimulating ? (
            <>
