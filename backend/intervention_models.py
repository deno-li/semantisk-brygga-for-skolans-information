...
class SemanticMappings(BaseModel):
    ...
    bbic: Optional[Dict[str, Any]] = Field(None, description="BBIC mapping with confidence")
    ibic: Optional[Dict[str, Any]] = Field(None, description="IBIC mapping (100% - ICF native)")
    kva: Optional[List[Dict[str, Any]]] = Field(None, description="KVÅ procedure codes")
    ss12000: Optional[Dict[str, Any]] = Field(None, description="SS 12000 programme mapping")
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "kva": [
                    {"code": "DV015", "text": "Rådgivning om studieteknik", "confidence": 0.85}
                ],
                "ss12000": {
...
class SupportIntervention(BaseModel):
    ...
    description_structured: Dict[str, Any] = Field(..., description="Structured intervention description")

    # Metadata
    source: InterventionSource = Field(..., description="Source of this intervention")
    shanarri_domain: Optional[SHANARRIDomain] = Field(None, description="Related SHANARRI domain")
    start_date: datetime = Field(..., description="Intervention start date")
    planned_end_date: Optional[datetime] = Field(None, description="Planned completion date")
    ...
    responsible_roles: List[str] = Field(..., description="Roles responsible for implementation")
    validated_by: Optional[List[str]] = Field(None, description="Professionals who validated")
    validation_date: Optional[datetime] = Field(None, description="When validated")

    # Confidence and mappings
    confidence: float = Field(..., ge=0.0, le=1.0, description="Overall confidence score")
    semantic_mappings: Optional[SemanticMappings] = Field(None, description="Mappings to other systems")

    # PDCA tracking
    pdca_phase: Optional[str] = Field(None, description="Current PDCA phase (Plan/Do/Check/Act)")
    effectiveness_rating: Optional[float] = Field(None, ge=0.0, le=10.0, description="How effective (0-10)")
...
class SurveyResponse(BaseModel):
    ...
    domain_scores: Dict[SHANARRIDomain, float] = Field(..., description="Scores 1-10 per domain")

    # Question-level responses
    question_responses: List[Dict[str, Any]] = Field(..., description="Individual question responses")

    # Free-text responses (kvalitativ statistik)
    freetext_responses: List[Dict[str, str]] = Field(default_factory=list, description="Open-ended responses")

    # AI analysis results
    ai_analysis: Optional[Dict[str, Any]] = Field(None, description="AI-generated ICF suggestions")
...
class SpiderChartData(BaseModel):
    ...
    scores: Dict[SHANARRIDomain, float] = Field(..., description="Scores for each domain")

    # Historical comparison
    previous_scores: Optional[Dict[SHANARRIDomain, float]] = Field(None, description="Previous period scores")
    change_delta: Optional[Dict[SHANARRIDomain, float]] = Field(None, description="Change since last period")

    # Related interventions
    active_interventions: List[str] = Field(default_factory=list, description="Active intervention IDs")
...
class PDCARecord(BaseModel):
    ...
    # Phase-specific data
    plan_data: Optional[Dict[str, Any]] = Field(None, description="Planning information")
    do_data: Optional[Dict[str, Any]] = Field(None, description="Implementation details")
...
class WelfareProfile(BaseModel):
    ...
    current_wellbeing: SpiderChartData = Field(..., description="Current wellbeing scores")

    # Active interventions
    active_interventions: List[SupportIntervention] = Field(default_factory=list, description="Current interventions")

    # ICF function descriptions
    icf_functions: List[Any] = Field(default_factory=list, description="ICF function descriptions")

    # Environmental factors
    environmental_factors: List[Any] = Field(default_factory=list, description="ICF environmental factors")

    # Historical data
    survey_history: List[SurveyResponse] = Field(default_factory=list, description="Survey responses over time")
    spider_chart_history: List[SpiderChartData] = Field(default_factory=list, description="Historical scores")

    # PDCA tracking
    pdca_records: List[PDCARecord] = Field(default_factory=list, description="PDCA cycle records")

    # Metadata
    last_review_date: Optional[datetime] = Field(None, description="Last professional review")
    next_review_date: Optional[datetime] = Field(None, description="Next scheduled review")
    responsible_team: List[str] = Field(default_factory=list, description="Responsible professionals")
